...
import {newcustomtest} from '../utils_ts/testbase';
import {POManager} from '../pageobjects_ts/POManager';
const dataSet = JSON.parse(JSON.stringify(require("../utils_ts/placeorderTestData.json")));  //convert Json -> string -> js object

for(const data of dataSet)
{ 
//change testnames for test data iterations
test(` @TS Cliont App Playwright test assignment ${data.productName}`, async ({page})=>
{    
...
newcustomtest(` @TS Cliont App Playwright test assignment`, async ({page, testDateOrder})=>
{    
    const poManager = new POManager(page);
    const loginPage = poManager.getLoginPage();
    const dashboardPage = poManager.getDashboardPage();
